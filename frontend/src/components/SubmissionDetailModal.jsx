...
import { AlertTriangle, CheckCircle2, Clock3, ExternalLink, FileText, ListChecks, LoaderCircle, Pencil, Send, Upload, XCircle } from "lucide-react";
...
export default function SubmissionDetailModal({ submissionId, account, onClose, onViewSummary }) {
  ...
  async function replaceSelected(event) {
    ...
    if (!file || !selectedFile || replacing) return;
    setReplacing(true); setError("");
    try { const response = await docVerificationApi.replace(submissionId, selectedFile.filename, file, detail.revision); setDetail((current) => ({ ...current, revision: response.revision, reanalysis_required: true, status: "NEEDS_HUMAN_REVIEW" })); }
    catch (err) { setError(err.message || "Could not replace document."); }
    finally { setReplacing(false); }
  }
  async function reanalyse() {
    if (!detail || reanalysing) return;
    setReanalysing(true); setError("");
    try { await docVerificationApi.reanalyse(submissionId, detail.revision); }
    catch (err) { setError(err.message || "Could not start reanalysis."); setReanalysing(false); }
  }
  if (reanalysing) return <div className="logModal" role="dialog" aria-modal="true"><div className="logModalPanel verificationModalPanel processingState"><LoaderCircle className="spin" size={32} /><strong>Reanalysing documents...</strong><span>All documents are being processed again. Keep this window open.</span></div></div>;
  return <div className="logModal" role="dialog" aria-modal="true"><div className="logModalPanel verificationModalPanel">
    <header className="logModalHeader"><div>{detail && <VerificationStatusBadge status={detail.status} />}<h2>{detail?.candidate_name || "Loading..."}</h2></div><button aria-label="Close" className="iconAction" disabled={replacing} onClick={onClose} type="button"><XCircle size={17} /></button></header>
      ...
      <section className="auditSummary">
        <div className="auditDecision">
          ...
          <button
            ...
            type="button"
            style={{ marginLeft: "auto", minHeight: "32px", padding: "6px 12px", fontSize: "13px" }}
          >
            Summarise candidate
          </button>
          {canWrite && detail.reanalysis_required && <button className="primaryAction" disabled={reanalysing} onClick={reanalyse} type="button"><LoaderCircle size={14} />Reanalyse</button>}
        </div>
        <dl className="auditMetaGrid">
          <div><dt><Clock3 size={14} />Last updated</dt><dd>{formatDateTime(detail.updated_at)}</dd></div>
          <div><dt><ListChecks size={14} />Issues found</dt><dd>{issues.length}</dd></div>
          <div><dt><FileText size={14} />Documents</dt><dd>{files.length}</dd></div>
          <div><dt><AlertTriangle size={14} />Pending</dt><dd>{(detail.pending_documents || []).length}</dd></div>
        </dl>
      </section>
      {(notes.length > 0 || confirmedChanges.length > 0) && <section className={`reviewNotesLayout ${confirmedChanges.length ? "withChanges" : ""}`}><div className="modalSection reviewNotesPanel"><div className="modalSectionHeader"><AlertTriangle size={16} /><h3>Review notes</h3></div>{notes.length ? <ul className="reviewNotesList">{notes.map((note, index) => <li className="reviewNoteItem issue" key={`${note}-${index}`}><AlertTriangle size={15} /><span>{note}</span></li>)}</ul> : <p className="emptyText">No review notes were generated.</p>}</div>{confirmedChanges.length > 0 && <div className="modalSection manualChangesPanel"><div className="modalSectionHeader"><h3>Changes made by user</h3></div><ul className="reviewNotesList">{confirmedChanges.map((change, index) => <li className="reviewNoteItem" key={`${change.filename}-${change.field}-${index}`}><CheckCircle2 size={15} /><span><strong>{change.filename}</strong>: {label(change.field)} changed to <strong>{valueText(change.to)}</strong> ({change.status === "match" ? "Matched" : "Mismatch"})</span></li>)}</ul></div>}</section>}
      <section className="verificationWorkspace">
        <aside className="documentGridPane">
          <div className="modalSectionHeader"><FileText size={16} /><h3>Submitted documents</h3></div>
          {files.length ? <div className="documentMiniGrid">
            {files.map((file) => {
              const extraction = extractions.find((item) => item.originalName === file.filename);
              let issue = documentIssueMap.get(file.filename);
              if (!issue && extraction?.extracted_data) {
                issue = Object.keys(extraction.extracted_data).some(
                  (field) => fieldState(extraction, field, issues) === "mismatch"
                );
              }
              return <button className={`documentMiniCard ${selectedFile?.filename === file.filename ? "selected" : ""}`} key={file.id} onClick={() => setSelectedFilename(file.filename)} type="button"><div className="docThumbWrap"><DocumentThumbnail filename={file.filename} url={file.url} /></div><span className="documentMiniTitle">{label(extraction?.document_type)}</span><span className="documentMiniMeta">{fileSize(file.size_bytes)}</span><span className={issue ? "badge badgeMismatch" : "badge badgeMatch"}>{issue ? "Review" : "Matched"}</span></button>;
            })}
          </div> : <p className="emptyText">No documents are available yet.</p>}
        </aside>
        <div className="documentReviewPane">
          <div className="documentPreviewPanel"><div className="documentPreviewHeader"><div><strong>{selectedFile?.filename || "No document selected"}</strong><span>{label(selectedExtraction?.document_type)}</span></div>{selectedFile && <div className="actionRow"><button className="secondaryAction" onClick={() => openFile(selectedFile.url)} type="button"><ExternalLink size={14} />Open</button>{canWrite && <label className="secondaryAction"><Upload size={14} />{replacing ? "Replacing..." : "Replace document"}<input accept=".pdf,image/*" hidden onChange={replaceSelected} type="file" /></label>}</div>}</div><div className="verificationPreviewSurface"><DocumentPreview file={selectedFile} /></div></div>
          <section className="modelFieldsPanel">
            <div className="modelFieldsHeader"><div className="modalSectionHeader"><ListChecks size={16} /><h3>Model extracted details</h3></div>{canWrite && selectedFields.length > 0 && <button className="secondaryAction" onClick={() => setEditing(true)} type="button"><Pencil size={14} />Edit</button>}</div>
            {selectedFields.length === 0 ? <p className="emptyPanelText">No extracted fields are available for this document yet.</p> : <div className="fieldResultGrid">{selectedFields.map(([field, value]) => { const match = fieldState(selectedExtraction, field, issues) === "match"; const Icon = match ? CheckCircle2 : AlertTriangle; return <div className={`fieldResultCard ${match ? "fieldMatch" : "fieldMismatch"}`} key={field}><div><span>{label(field)}</span><strong>{valueText(value)}</strong></div><span className="fieldResultBadge"><Icon size={14} />{match ? "Matched" : "Mismatch"}</span></div>; })}</div>}
            {draftChanges.length > 0 && <div className="actionRow confirmChangesRow"><span>{draftChanges.length} reviewed field{draftChanges.length === 1 ? "" : "s"} waiting for confirmation.</span><button className="primaryAction" disabled={saving} onClick={confirmChanges} type="button"><Send size={15} />{saving ? "Confirming..." : "Confirm submission"}</button></div>}
          </section>
        </div>
      </section>
    </>}
